...
def _extra_samples_for(json_path: Path) -> list[Path]:
    """Repo-owned supplementary samples merged into schema inference.

    Real Slack responses ship shapes the upstream java-slack-sdk samples
    don't cover (e.g. raw strings under ``blocks[].elements[].text`` and
    ``blocks[].title`` for rich_text / third-party app unfurls). Drop a JSON
    file at ``samples/<endpoint-path>/<stem>/*.json`` and ``generate_module``
    feeds it to genson alongside the primary sample so the inferred schema
    captures the polymorphism.
    """
    try:
        rel = json_path.relative_to(JSON_ROOT)
    except ValueError:
        return []
    extra_dir = EXTRA_ROOT / rel.with_suffix("")
...
def generate_module(json_path: Path, root_class: str, out_path: Path) -> None:
    ...
    for extra in _extra_samples_for(json_path):
        builder.add_object(json.loads(extra.read_text()))
