...
#
#     result = workflow_published_payload_from_dict(json.loads(json_string))
...
class WorkflowPublishedConfiguration:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'WorkflowPublishedConfiguration':
        assert isinstance(obj, dict)
        version_id = from_union([from_str, from_none], obj.get("version_id"))
        return WorkflowPublishedConfiguration(version_id)
...
class Event:
    ...
    workflow_published_configuration: Optional[WorkflowPublishedConfiguration] = None
    ...
    def from_dict(obj: Any) -> 'Event':
        ...
        workflow_published_configuration = from_union([WorkflowPublishedConfiguration.from_dict, from_none], obj.get("workflow_published_configuration"))
        event_ts = from_union([from_str, from_none], obj.get("event_ts"))
        return Event(type, workflow_id, workflow_published_configuration, event_ts)

    def to_dict(self) -> dict:
        result: dict = {}
        result["type"] = from_union([from_str, from_none], self.type)
        result["workflow_id"] = from_union([from_str, from_none], self.workflow_id)
        result["workflow_published_configuration"] = from_union([lambda x: to_class(WorkflowPublishedConfiguration, x), from_none], self.workflow_published_configuration)
        result["event_ts"] = from_union([from_str, from_none], self.event_ts)
        return result


@dataclass
class WorkflowPublishedPayload:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'WorkflowPublishedPayload':
        assert isinstance(obj, dict)
        ...
        return WorkflowPublishedPayload(token, enterprise_id, team_id, api_app_id, type, authed_users, authed_teams, authorizations, is_ext_shared_channel, event_id, event_time, event_context, event)
...
def workflow_published_payload_from_dict(s: Any) -> WorkflowPublishedPayload:
    return WorkflowPublishedPayload.from_dict(s)


def workflow_published_payload_to_dict(x: WorkflowPublishedPayload) -> Any:
    return to_class(WorkflowPublishedPayload, x)
