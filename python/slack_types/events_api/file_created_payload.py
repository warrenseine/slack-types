...
#
#     result = file_created_payload_from_dict(json.loads(json_string))
...
class FileCreatedPayload:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'FileCreatedPayload':
        assert isinstance(obj, dict)
        ...
        return FileCreatedPayload(token, team_id, enterprise_id, api_app_id, event, type, event_id, event_time, authorizations, is_ext_shared_channel, event_context)
...
def file_created_payload_from_dict(s: Any) -> FileCreatedPayload:
    return FileCreatedPayload.from_dict(s)


def file_created_payload_to_dict(x: FileCreatedPayload) -> Any:
    return to_class(FileCreatedPayload, x)
