...
#
#     result = message_me_payload_from_dict(json.loads(json_string))
...
class MessageMePayload:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'MessageMePayload':
        assert isinstance(obj, dict)
        ...
        return MessageMePayload(token, enterprise_id, team_id, api_app_id, type, authed_users, authed_teams, authorizations, is_ext_shared_channel, event_id, event_time, event_context, event)
...
def message_me_payload_from_dict(s: Any) -> MessageMePayload:
    return MessageMePayload.from_dict(s)


def message_me_payload_to_dict(x: MessageMePayload) -> Any:
    return to_class(MessageMePayload, x)
