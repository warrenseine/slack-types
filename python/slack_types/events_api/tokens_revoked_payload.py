...
#
#     result = tokens_revoked_payload_from_dict(json.loads(json_string))
...
class Tokens:
    ...
    def from_dict(obj: Any) -> 'Tokens':
        assert isinstance(obj, dict)
        oauth = from_union([lambda x: from_list(from_str, x), from_none], obj.get("oauth"))
        bot = from_union([lambda x: from_list(from_str, x), from_none], obj.get("bot"))
        return Tokens(oauth, bot)

    def to_dict(self) -> dict:
        result: dict = {}
        result["oauth"] = from_union([lambda x: from_list(from_str, x), from_none], self.oauth)
...
class Event:
    ...
    def from_dict(obj: Any) -> 'Event':
        ...
        tokens = from_union([Tokens.from_dict, from_none], obj.get("tokens"))
        return Event(type, tokens)

    def to_dict(self) -> dict:
        result: dict = {}
        result["type"] = from_union([from_str, from_none], self.type)
        result["tokens"] = from_union([lambda x: to_class(Tokens, x), from_none], self.tokens)
        return result


@dataclass
class TokensRevokedPayload:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'TokensRevokedPayload':
        assert isinstance(obj, dict)
        ...
        return TokensRevokedPayload(token, enterprise_id, team_id, api_app_id, type, authed_users, authed_teams, authorizations, is_ext_shared_channel, event_id, event_time, event_context, event)
...
def tokens_revoked_payload_from_dict(s: Any) -> TokensRevokedPayload:
    return TokensRevokedPayload.from_dict(s)


def tokens_revoked_payload_to_dict(x: TokensRevokedPayload) -> Any:
    return to_class(TokensRevokedPayload, x)
