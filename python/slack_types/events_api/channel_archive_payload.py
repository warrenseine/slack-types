...
#
#     result = channel_archive_payload_from_dict(json.loads(json_string))
...
class ChannelArchivePayload:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'ChannelArchivePayload':
        assert isinstance(obj, dict)
        ...
        return ChannelArchivePayload(token, team_id, enterprise_id, api_app_id, event, type, event_id, event_time, authorizations, is_ext_shared_channel, event_context)
...
def channel_archive_payload_from_dict(s: Any) -> ChannelArchivePayload:
    return ChannelArchivePayload.from_dict(s)


def channel_archive_payload_to_dict(x: ChannelArchivePayload) -> Any:
    return to_class(ChannelArchivePayload, x)
