...
#
#     result = scope_denied_payload_from_dict(json.loads(json_string))
...
class ScopeDeniedPayload:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'ScopeDeniedPayload':
        assert isinstance(obj, dict)
        ...
        return ScopeDeniedPayload(token, enterprise_id, team_id, api_app_id, type, authed_users, authed_teams, authorizations, is_ext_shared_channel, event_id, event_time, event_context, event)
...
def scope_denied_payload_from_dict(s: Any) -> ScopeDeniedPayload:
    return ScopeDeniedPayload.from_dict(s)


def scope_denied_payload_to_dict(x: ScopeDeniedPayload) -> Any:
    return to_class(ScopeDeniedPayload, x)
