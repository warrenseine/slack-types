...
#
#     result = message_deleted_payload_from_dict(json.loads(json_string))
...
class PreviousMessage:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'PreviousMessage':
        assert isinstance(obj, dict)
        ...
        return PreviousMessage(client_msg_id, type, subtype, user, team, edited, text, blocks, attachments, upload, display_as_bot, thread_ts, parent_user_id, bot_id, bot_profile, hidden, is_locked, subscribed, ts, user_team, source_team, is_starred, reply_count, reply_users_count, latest_reply, last_read)
...
class Event:
    ...
    previous_message: Optional[PreviousMessage] = None
    ...
    def from_dict(obj: Any) -> 'Event':
        ...
        hidden = from_union([from_bool, from_none], obj.get("hidden"))
        deleted_ts = from_union([from_str, from_none], obj.get("deleted_ts"))
        previous_message = from_union([PreviousMessage.from_dict, from_none], obj.get("previous_message"))
        event_ts = from_union([from_str, from_none], obj.get("event_ts"))
        ts = from_union([from_str, from_none], obj.get("ts"))
        channel_type = from_union([from_str, from_none], obj.get("channel_type"))
        return Event(type, subtype, channel, hidden, deleted_ts, previous_message, event_ts, ts, channel_type)
    ...
    def to_dict(self) -> dict:
        ...
        result["hidden"] = from_union([from_bool, from_none], self.hidden)
        result["deleted_ts"] = from_union([from_str, from_none], self.deleted_ts)
        result["previous_message"] = from_union([lambda x: to_class(PreviousMessage, x), from_none], self.previous_message)
...
class MessageDeletedPayload:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'MessageDeletedPayload':
        assert isinstance(obj, dict)
        ...
        return MessageDeletedPayload(token, enterprise_id, team_id, api_app_id, type, authed_users, authed_teams, authorizations, is_ext_shared_channel, event_id, event_time, event_context, event)
...
def message_deleted_payload_from_dict(s: Any) -> MessageDeletedPayload:
    return MessageDeletedPayload.from_dict(s)


def message_deleted_payload_to_dict(x: MessageDeletedPayload) -> Any:
    return to_class(MessageDeletedPayload, x)
