...
class TeamRenamePayload:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'TeamRenamePayload':
        assert isinstance(obj, dict)
        ...
        return TeamRenamePayload(token, enterprise_id, team_id, api_app_id, type, authed_users, authed_teams, authorizations, is_ext_shared_channel, event_id, event_time, event_context, event)
...
def team_rename_payload_from_dict(s: Any) -> TeamRenamePayload:
    return TeamRenamePayload.from_dict(s)


def team_rename_payload_to_dict(x: TeamRenamePayload) -> Any:
    return to_class(TeamRenamePayload, x)
