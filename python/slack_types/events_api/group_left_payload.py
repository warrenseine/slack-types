...
#
#     result = group_left_payload_from_dict(json.loads(json_string))
...
class GroupLeftPayload:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'GroupLeftPayload':
        assert isinstance(obj, dict)
        ...
        return GroupLeftPayload(token, team_id, enterprise_id, api_app_id, event, type, event_id, event_time, authorizations, is_ext_shared_channel, event_context)
...
def group_left_payload_from_dict(s: Any) -> GroupLeftPayload:
    return GroupLeftPayload.from_dict(s)


def group_left_payload_to_dict(x: GroupLeftPayload) -> Any:
    return to_class(GroupLeftPayload, x)
