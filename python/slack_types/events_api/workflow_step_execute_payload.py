...
#
#     result = workflow_step_execute_payload_from_dict(json.loads(json_string))
...
@dataclass
class WorkflowStep:
    workflow_step_execute_id: Optional[str] = None
    workflow_id: Optional[str] = None
    workflow_instance_id: Optional[str] = None
    step_id: Optional[str] = None

    @staticmethod
    def from_dict(obj: Any) -> 'WorkflowStep':
        assert isinstance(obj, dict)
        workflow_step_execute_id = from_union([from_str, from_none], obj.get("workflow_step_execute_id"))
        workflow_id = from_union([from_str, from_none], obj.get("workflow_id"))
        workflow_instance_id = from_union([from_str, from_none], obj.get("workflow_instance_id"))
        step_id = from_union([from_str, from_none], obj.get("step_id"))
        return WorkflowStep(workflow_step_execute_id, workflow_id, workflow_instance_id, step_id)

    def to_dict(self) -> dict:
        result: dict = {}
        result["workflow_step_execute_id"] = from_union([from_str, from_none], self.workflow_step_execute_id)
        result["workflow_id"] = from_union([from_str, from_none], self.workflow_id)
        result["workflow_instance_id"] = from_union([from_str, from_none], self.workflow_instance_id)
...
class Event:
    ...
    def from_dict(obj: Any) -> 'Event':
        ...
        workflow_step = from_union([WorkflowStep.from_dict, from_none], obj.get("workflow_step"))
        event_ts = from_union([from_str, from_none], obj.get("event_ts"))
        return Event(type, callback_id, workflow_step, event_ts)

    def to_dict(self) -> dict:
        result: dict = {}
        result["type"] = from_union([from_str, from_none], self.type)
        result["callback_id"] = from_union([from_str, from_none], self.callback_id)
        result["workflow_step"] = from_union([lambda x: to_class(WorkflowStep, x), from_none], self.workflow_step)
...
class WorkflowStepExecutePayload:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'WorkflowStepExecutePayload':
        assert isinstance(obj, dict)
        ...
        return WorkflowStepExecutePayload(token, enterprise_id, team_id, api_app_id, type, authed_users, authed_teams, authorizations, is_ext_shared_channel, event_id, event_time, event_context, event)
...
def workflow_step_execute_payload_from_dict(s: Any) -> WorkflowStepExecutePayload:
    return WorkflowStepExecutePayload.from_dict(s)


def workflow_step_execute_payload_to_dict(x: WorkflowStepExecutePayload) -> Any:
    return to_class(WorkflowStepExecutePayload, x)
