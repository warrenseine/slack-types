...
#
#     result = im_created_payload_from_dict(json.loads(json_string))
...
class IMCreatedPayload:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'IMCreatedPayload':
        assert isinstance(obj, dict)
        ...
        return IMCreatedPayload(token, enterprise_id, team_id, api_app_id, type, authed_users, authed_teams, authorizations, is_ext_shared_channel, event_id, event_time, event_context, event)
...
def im_created_payload_from_dict(s: Any) -> IMCreatedPayload:
    return IMCreatedPayload.from_dict(s)


def im_created_payload_to_dict(x: IMCreatedPayload) -> Any:
    return to_class(IMCreatedPayload, x)
