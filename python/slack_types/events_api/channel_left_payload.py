...
#
#     result = channel_left_payload_from_dict(json.loads(json_string))
...
class ChannelLeftPayload:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'ChannelLeftPayload':
        assert isinstance(obj, dict)
        ...
        return ChannelLeftPayload(token, team_id, enterprise_id, api_app_id, event, type, event_id, event_time, authorizations, is_ext_shared_channel, event_context)
...
def channel_left_payload_from_dict(s: Any) -> ChannelLeftPayload:
    return ChannelLeftPayload.from_dict(s)


def channel_left_payload_to_dict(x: ChannelLeftPayload) -> Any:
    return to_class(ChannelLeftPayload, x)
