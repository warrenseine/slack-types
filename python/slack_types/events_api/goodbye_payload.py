...
#
#     result = goodbye_payload_from_dict(json.loads(json_string))
...
class GoodbyePayload:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'GoodbyePayload':
        assert isinstance(obj, dict)
        ...
        return GoodbyePayload(token, enterprise_id, team_id, api_app_id, type, authed_users, authed_teams, authorizations, is_ext_shared_channel, event_id, event_time, event_context, event)
...
def goodbye_payload_from_dict(s: Any) -> GoodbyePayload:
    return GoodbyePayload.from_dict(s)


def goodbye_payload_to_dict(x: GoodbyePayload) -> Any:
    return to_class(GoodbyePayload, x)
