...
#
#     result = team_join_payload_from_dict(json.loads(json_string))
...
class TeamJoinPayload:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'TeamJoinPayload':
        assert isinstance(obj, dict)
        ...
        return TeamJoinPayload(token, enterprise_id, team_id, api_app_id, type, authed_users, authed_teams, authorizations, is_ext_shared_channel, event_id, event_time, event_context, event)
...
def team_join_payload_from_dict(s: Any) -> TeamJoinPayload:
    return TeamJoinPayload.from_dict(s)


def team_join_payload_to_dict(x: TeamJoinPayload) -> Any:
    return to_class(TeamJoinPayload, x)
