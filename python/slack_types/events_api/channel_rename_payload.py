...
#
#     result = channel_rename_payload_from_dict(json.loads(json_string))
...
class ChannelRenamePayload:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'ChannelRenamePayload':
        assert isinstance(obj, dict)
        ...
        return ChannelRenamePayload(token, team_id, enterprise_id, api_app_id, event, type, event_id, event_time, authorizations, is_ext_shared_channel, event_context)
...
def channel_rename_payload_from_dict(s: Any) -> ChannelRenamePayload:
    return ChannelRenamePayload.from_dict(s)


def channel_rename_payload_to_dict(x: ChannelRenamePayload) -> Any:
    return to_class(ChannelRenamePayload, x)
