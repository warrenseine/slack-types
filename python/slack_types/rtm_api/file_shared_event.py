...
#
#     result = file_shared_event_from_dict(json.loads(json_string))
...
class FileSharedEvent:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'FileSharedEvent':
        assert isinstance(obj, dict)
        ...
        return FileSharedEvent(type, file_id, file, channel_id, user_id, event_ts)
...
def file_shared_event_from_dict(s: Any) -> FileSharedEvent:
    return FileSharedEvent.from_dict(s)


def file_shared_event_to_dict(x: FileSharedEvent) -> Any:
    return to_class(FileSharedEvent, x)
