...
#
#     result = message_event_from_dict(json.loads(json_string))
...
class MessageEvent:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'MessageEvent':
        assert isinstance(obj, dict)
        ...
        return MessageEvent(client_msg_id, type, team, channel, user, bot_id, bot_profile, text, blocks, attachments, files, ts, parent_user_id, thread_ts, event_ts, channel_type, edited)
...
def message_event_from_dict(s: Any) -> MessageEvent:
    return MessageEvent.from_dict(s)


def message_event_to_dict(x: MessageEvent) -> Any:
    return to_class(MessageEvent, x)
