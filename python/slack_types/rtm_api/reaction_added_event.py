...
#
#     result = reaction_added_event_from_dict(json.loads(json_string))
...
class ReactionAddedEvent:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'ReactionAddedEvent':
        assert isinstance(obj, dict)
        type = from_union([from_str, from_none], obj.get("type"))
        ...
        return ReactionAddedEvent(type, user, reaction, item_user, item, event_ts)
...
def reaction_added_event_from_dict(s: Any) -> ReactionAddedEvent:
    return ReactionAddedEvent.from_dict(s)


def reaction_added_event_to_dict(x: ReactionAddedEvent) -> Any:
    return to_class(ReactionAddedEvent, x)
