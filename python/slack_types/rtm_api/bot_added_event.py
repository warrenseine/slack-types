...
#
#     result = bot_added_event_from_dict(json.loads(json_string))
...
class BotAddedEvent:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'BotAddedEvent':
        assert isinstance(obj, dict)
        type = from_union([from_str, from_none], obj.get("type"))
        bot = from_union([Bot.from_dict, from_none], obj.get("bot"))
        return BotAddedEvent(type, bot)
...
def bot_added_event_from_dict(s: Any) -> BotAddedEvent:
    return BotAddedEvent.from_dict(s)


def bot_added_event_to_dict(x: BotAddedEvent) -> Any:
    return to_class(BotAddedEvent, x)
