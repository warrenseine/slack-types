...
#
#     result = im_replies_response_from_dict(json.loads(json_string))
...
class IMRepliesResponse:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'IMRepliesResponse':
        assert isinstance(obj, dict)
        ...
        return IMRepliesResponse(messages, has_more, ok, warning, response_metadata, error, needed, provided)
...
def im_replies_response_from_dict(s: Any) -> IMRepliesResponse:
    return IMRepliesResponse.from_dict(s)


def im_replies_response_to_dict(x: IMRepliesResponse) -> Any:
    return to_class(IMRepliesResponse, x)
